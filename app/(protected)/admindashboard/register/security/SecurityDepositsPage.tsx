...
import { Clock, IndianRupee, CalendarCheck, FileText, Check, Filter, Download, Search, ChevronDown, AlertTriangle } from "lucide-react"
    ...
    return (
      <TableRow className="group border-b border-slate-100 hover:bg-slate-50/80 transition-all duration-200">
        ...
        <TableCell className="px-6 py-4">
          <div className="font-semibold text-slate-900 line-clamp-2 max-w-[250px]">{bidAgency}</div>
        </TableCell>
        <TableCell className="px-6 py-4">
          {nitDetails && (
            <ShowNitDetails
        ...
        <TableCell className="px-6 py-4 text-right">
          {currentStatus === "unpaid" && <MarkPaidButton depositId={deposit.id} />}
        </TableCell>
      </TableRow>
    )
  },
...
export function SecurityDepositsPage({ deposits }: SecurityDepositsPageProps) {
  ...
  const exportToPDF = useCallback(() => {
    ...
    try {
      ...
      doc.text("Security Deposits Report", pageWidth / 2, 20, { align: "center" })
      ...
      const tableData = filteredDeposits.map((deposit, index) => {
        ...
        return [
          ...
          maturityDate ? formatDate(maturityDate) : "N/A",
          daysRemaining === null ? "N/A" : daysRemaining < 0 ? "Matured" : `${daysRemaining} days`,
          currentStatus === "paid" ? "Paid" : "unpaid",
        ]
      })
      ...
      autoTable(doc, {
        ...
        startY: 70,
        theme: "striped",
        headStyles: { fillColor: [79, 70, 229], textColor: 255, fontStyle: "bold", fontSize: 10, halign: "center" },
      ...
      doc.save(`security-deposits-${new Date().toISOString().slice(0, 10)}.pdf`)
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-12 text-slate-900">
      <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8 max-w-7xl space-y-8">
        ...
        >
          ...
          <Card className="border-0 shadow-sm ring-1 ring-slate-200 overflow-hidden bg-white">
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div className="space-y-1">
                  <p className="text-sm font-medium text-slate-500">Active Deposits</p>
                  <h3 className="text-2xl font-bold tracking-tight text-slate-900">
                    {summary.activeCount}
                  </h3>
                </div>
          ...
          <Card className="border-0 shadow-sm ring-1 ring-slate-200 overflow-hidden bg-white">
            <CardContent className="p-6">
              <div className="flex justify-between items-center">
                <div className="space-y-1">
                  <p className="text-sm font-medium text-slate-500">Approaching Maturity</p>
        ...
        <motion.div
          ...
          className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-4 items-center"
        >
          ...
          <Card className="border-0 shadow-sm ring-1 ring-slate-200 rounded-2xl overflow-hidden bg-white">
            <CardHeader className="border-b border-slate-100 py-5 bg-white">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary" className="bg-slate-100 text-slate-600 hover:bg-slate-100 font-medium px-3">
              ...
              {filteredDeposits.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-50/30">
                  ...
                  <p className="text-slate-500 max-w-sm text-sm">
                    We couldn&apos;t find any deposits matching your current filters and search term.
                  </p>
                  <Button 
                    variant="link" 
                    onClick={() => { setSearchQuery(""); setSelectedFund("all"); setStatusFilter("all"); }}
                  ...
                  <Table>
                    <TableHeader className="bg-slate-50/80 border-b border-slate-200">
                      <TableRow className="hover:bg-transparent">
                        ...
                        <TableHead className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">#</TableHead>
                        <TableHead className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Agency</TableHead>
                        <TableHead className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">NIT Details</TableHead>
                        <TableHead className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 text-right">Amount</TableHead>
                        <TableHead className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Maturity</TableHead>
