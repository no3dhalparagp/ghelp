...
// ========== CENTRALIZED QUANTITY CALCULATOR (consistent with AddEstimateItemCard) ==========
...
function calcQty(
  ...
  switch (unit?.toLowerCase()) {
    ...
    default:
      ...
      // This maintains some backward compatibility for general items
  ...
  const handleAddMain = () => {
    setMeasurements((prev) => [
      ...prev,
      {
        id: crypto.randomUUID(),
        description: estimateItem?.description || "",
        nos: estimateItem?.nos ?? 0,
        length: estimateItem?.length ?? 0,
        breadth: estimateItem?.breadth ?? 0,
        depth: estimateItem?.depth ?? 0,
        quantity: 0,
        estimateItemId: estimateItem?.id,
        isSubItem: false,
      },
  ...
  const handleChange = (
    ...
    setMeasurements((prev) =>
      prev.map((m) => {
        ...
        const updated = { ...m, [field]: numValue } as Measurement;
