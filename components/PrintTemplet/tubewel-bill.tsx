  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      for (let pageIdx = 0; pageIdx < chunks.length; pageIdx++) {
        ...
        if (isLastMasterPage) {
          chunkBody.push([
            ...
            { content: bill.totalLaborCost.toFixed(2), styles: { fontStyle: "bold", fillColor: [248, 250, 252] } },
          ]);
        }
      ...
      if (materials.length > 0) {
        // Estimate required height for two columns
        const rowsPerColumn = Math.ceil(materials.length / 2);
        const tableHeight = rowsPerColumn * 7 + 15; // approximate height per column
        ...
        const leftMaterials = materials.slice(0, mid);
        const rightMaterials = materials.slice(mid);

        const gap = 5; // gap between columns in mm
        const colWidth = (pageWidth - 2 * margin - gap) / 2;

        // Helper to build table body for a materials array
        const buildMaterialsBody = (matArray: any[], startIndex: number) => {
        ...
        const leftBody = buildMaterialsBody(leftMaterials, 0);
        const rightBody = buildMaterialsBody(rightMaterials, leftMaterials.length);
        ...
        (doc as any).autoTable({
          ...
          body: leftBody,
          startY: finalY,
          margin: { left: margin, right: margin + colWidth + gap },
          tableWidth: colWidth,
          styles: {
            fontSize: 8,
            cellPadding: 1.5,
        ...
        // Capture finalY of left table before drawing right
        ...
        (doc as any).autoTable({
          ...
          body: rightBody,
          startY: finalY,
          margin: { left: margin + colWidth + gap, right: margin },
          tableWidth: colWidth,
          styles: {
            fontSize: 8,
            cellPadding: 1.5,
        ...
        finalY = Math.max(leftFinalY, rightFinalY) + 5;

        // Subtotal row (spans both columns)
        doc.setFontSize(9);
        doc.setFont("helvetica", "bold");
        const subtotalText = `Subtotal (Materials): ${bill.totalMaterialCost.toFixed(2)}`;
      ...
      doc.setFontSize(14);
      doc.text(
        `INR ${bill.netAmount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
