...
async function generateAcknowledgmentNumber(gpProfileId: string) {
  const gpProfile = await db.gPProfile.findUnique({
    where: { id: gpProfileId },
    select: { gpnameinshort: true, gpcode: true },
  });

  if (!gpProfile) throw new Error("GP Profile not found");
  ...
  const acknowledgmentNumber = `ACK/${gpProfile.gpcode}/${currentYear}/${sequenceNumber
    .toString()
    .padStart(4, "0")}`;
...
export async function createNestedWarishDetails(
  ...
  const gpProfileId = user.gpProfileId;
  if (!gpProfileId) throw new Error("GP Profile ID not found for user");

  try {
    // Validate input
    const acknowlegment = await generateAcknowledgmentNumber(gpProfileId);
...
export async function submitEnquiryReport(formData: FormData) {
  ...
  try {
    ...
    const data = await db.warishApplication.update({
      where: {
        id: applicationId, gpProfileId: user.gpProfileId, // Scope by GP
      },
      data: {
...
export async function approvedWarishApplication(
  ...
  try {
    ...
    const updatedApplication = await db.warishApplication.update({
      where: {
        id: applicationId,
        gpProfileId: user.gpProfileId, // Scope by GP
      },
      data: updateData,
    });
...
async function createUserNotification(
  ...
  const notificationMessage = `Your Warish application has been ${validatedData.status
...
export async function getLatestMemoNumber(year: number): Promise<number> {
  try {
    const user = await currentUser();
    if (!user || !user.id || !user.gpProfileId) {
      throw new Error("User not authenticated or GP Profile not found");
    }
...
export async function searchWarishApplications(searchParams: {
  ...
  // Always scope by current user's GP
  if (user.gpProfileId) {
    conditions.push({ gpProfileId: user.gpProfileId });
  }
...
export async function assignStaff(formData: FormData) {
  ...
  try {
    ...
    if (
      ...
      throw new Error("Staff or application not found or access denied");
    }
...
export async function bulkAssignStaff(formData: FormData) {
  ...
  try {
    ...
    if (applications.length === 0) {
      throw new Error("No valid applications found or access denied");
    }
...
export async function getWarishByAck(
  ...
  try {
    ...
    return {
      ...
      gpProfileId: application.gpProfileId!,
...
export async function updateNestedWarishDetails(
  ...
  try {
    ...
    return await db.$transaction(async (tx) => {
      // Ensure the application belongs to the same GP
      const existingApplication = await tx.warishApplication.findFirst({
        where: { id: ackNumber, gpProfileId: gpProfileId },
      });
