...
async function handleFailedLogin(
  ...
  // Use provided location or fallback to fetching it
  const resolvedLocation = location || await getLocationFromIP(ipAddress);

  // Record the failed attempt FIRST before anything else
  await recordLoginAttempt(user.email, ipAddress, false, userAgent, resolvedLocation);
...
export const login = async (
  ...
  if (!existingUser || !existingUser.email || !existingUser.password) {
    await recordLoginAttempt(email, ipAddress, false, userAgent, location);
    return {
  ...
  if (!passwordMatch) {
    return await handleFailedLogin(existingUser, ipAddress, userAgent, location);
  }
  ...
  try {
    ...
    await recordLoginAttempt(email, ipAddress, true, userAgent, location);
